...
/**
 * HTTP Server Factory for NiFi FlowFile Transfer
 */
public class HttpServerFactory extends NettyEventServerFactory {

    private static final String[] APPLICATION_PROTOCOLS = {ApplicationProtocolNames.HTTP_2};

    private static final Set<String> SUPPORTED_CIPHER_SUITES = new LinkedHashSet<>(Http2SecurityUtil.CIPHERS);

    public HttpServerFactory(final ComponentLog log, final StreamFrameCallback streamFrameCallback, final InetAddress address, final int port, final SSLContext sslContext) {
        ...
        setHandlerSupplier(() -> Arrays.asList(
                new HttpProtocolNegotiationHandler(log, streamFrameCallback),
