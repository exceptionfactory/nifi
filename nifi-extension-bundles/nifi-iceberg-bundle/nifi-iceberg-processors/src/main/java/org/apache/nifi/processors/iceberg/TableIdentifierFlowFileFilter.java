...
class TableIdentifierFlowFileFilter implements FlowFileFilter {
    ...
    private TableIdentifier getFlowFileTableIdentifier(final FlowFile flowFile) {
        final String namespace = context.getProperty(PutIcebergTable.NAMESPACE).evaluateAttributeExpressions(flowFile).getValue();
        final String tableName = context.getProperty(PutIcebergTable.TABLE_NAME).evaluateAttributeExpressions(flowFile).getValue();
